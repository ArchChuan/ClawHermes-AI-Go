...
# 或回滚到特定版本
bash scripts/deploy/rollback.sh clawhermes 2
```
...
# 特定 Pod（需要 Pod 名称）
kubectl logs -n clawhermes POD_NAME -f
...
kubectl exec -it $POD -n clawhermes -- sh
```
...
# 或更详细的信息
kubectl top pods -n clawhermes -l app=clawhermes-ai --containers
...
# 查看事件
kubectl get events -n clawhermes --sort-by='.lastTimestamp'
...
# 检查镜像是否存在
eval $(minikube docker-env)
docker images | grep clawhermes

# 重新构建镜像
make docker-build-minikube

# 删除并重新部署
kubectl delete deployment clawhermes-ai -n clawhermes
make local-deploy
```
...
所有配置通过 `clawhermes-config` ConfigMap 管理，存储在：
```bash
kubectl get configmap clawhermes-config -n clawhermes -o yaml
```

修改配置：
```bash
kubectl edit configmap clawhermes-config -n clawhermes
# 然后重启 Pod
kubectl rollout restart deployment/clawhermes-ai -n clawhermes
```
...
```bash
kubectl scale deployment/clawhermes-ai -n clawhermes --replicas=3
```

**Q: 如何模拟生产环境？**
A: 使用生产 values 文件：
```bash
helm install clawhermes ./charts/clawhermes-ai -n clawhermes -f charts/clawhermes-ai/values-prod.yaml
```
